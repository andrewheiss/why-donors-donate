...
Causality can be inferred through conjoint analysis because it shows which components of a choice hold the most weight. Conjoint analysis has been widely used within social sciences and has recently begun to be used in political and public policy fields. Helfer (2016) and Mummolo (2016) both used conjoint analysis to assess tradeoffs made in political communications. ### rewrite in more detail so that it's clear that consumers are making the tradeoffs ### Conjoint is key in political research because it captures multidimensional causal relations.  

Knudsen and Johannesson (2018) argue that political communication research requires conjoint analysis because it accounts for the masking effects of factors as well as the complexity of a respondent's judgement of credibility. Public policy research would benefit from conjoint analysis for the same reasons.

# Results

### First Set of Questions
The first set of questions (hypotheses) can be answered using the results of the intercept-only model.

We hypothesized that each of these structural and organizational characteristics would have a significant effect on donor behavior, though we were unsure of the direction of that effect.

![](output/figures/combined-results.png)



The following plots zoom in on the strength of the effects of these characteristics.

Compared to when an NGO has a friendly relationship with the host government, donors who see that an NGO faces general legal trouble abroad are less likely to donate.



![](output/figures/relationship-with-gov.png)

Donors are more likely to donate when organizations are accountable and financially transparent.

![](output/figures/managerial-practices.png)

Donors are most likely to donate when an NGO is funded primarily by many small private donations. Donors are less likely to donate when the organization is subsidized by government grants, and even less likely when it is funded by a few private donors.



![](output/figures/funding-sources.png)

Of all issue areas, donations to emergency response were most likely, followed by human rights and then environment. Refugee relief was the least likely issue area to be donated to.



![](output/figures/issue-areas.png)

All organizations were positively percieved by respondents. The Red Cross was significantly more favored.


![](output/figures/organization.png)
...
The second set of questions (confirmatory) can be answered using the following upper-level models:
  - Public affairs: `Donation preferences ~ Public affairs knowledge + Public affairs activity`
    ...
    - Traveled Internationally: No
      ![](output/figures/traveled.png)
    
    - Medium to follow news: Online (excluding social media) with all attributes
      ![](output/figures/following-news.png)
    
    - Medium to follow news: Online (excluding social media) with organization
...
The third set of questions (exploratory) can be answered using a market simulator.
