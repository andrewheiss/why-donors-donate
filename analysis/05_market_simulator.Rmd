...
p1_politic_pref <- reactive({
  max(input$p1_political_pref)
})
...
p <- reactive({
  ...
  #Persona 1
    ...
    # beta_persona$p1_value[beta_persona$levels == "Political Preference"] <- 1
...
renderGauge({
  ...
  ai_exp_util <- round(exp(t(choice[, 3]) %*% as.matrix(p_beta1)),2)
  gp_exp_util <- round(exp(t(choice[, 4]) %*% as.matrix(p_beta1)),2)
  ox_exp_util <- round(exp(t(choice[, 5]) %*% as.matrix(p_beta1)),2)
  rc_exp_util <- round(exp(t(choice[, 6]) %*% as.matrix(p_beta1)),2)
...
renderGauge({
  ...
  ai_exp_util <- round(exp(t(choice[, 3]) %*% as.matrix(p_beta2)),2)
  gp_exp_util <- round(exp(t(choice[, 4]) %*% as.matrix(p_beta2)),2)
  ox_exp_util <- round(exp(t(choice[, 5]) %*% as.matrix(p_beta2)),2)
  rc_exp_util <- round(exp(t(choice[, 6]) %*% as.matrix(p_beta2)),2)
...
renderGauge({
  ...
  ai_exp_util <- round(exp(t(choice[, 3]) %*% as.matrix(p_beta3)),2)
  gp_exp_util <- round(exp(t(choice[, 4]) %*% as.matrix(p_beta3)),2)
  ox_exp_util <- round(exp(t(choice[, 5]) %*% as.matrix(p_beta3)),2)
  rc_exp_util <- round(exp(t(choice[, 6]) %*% as.matrix(p_beta3)),2)
...
selectInput("p1_travel_developing",
         label = "Travel to developing country:",
         choices = c("Yes", 'No' = "No Travel to Developing Countries"))
...
<!-- This is for commented out code -->
