...
renderPlotly({
  ...
  per2_org3_exp <- exp(t(org_choice[,6]) %*% as.matrix(per2_betas))
  ...
  # Empty organization reactive table
  ...
  org_reactive[,3] <- c(rep('Expected Utility', 4), rep('Market Share', 4))
  # Persona name inputs
  ...
  ggplotly(
    org_reactive %>% 
      ...
      mutate(
        Organization = factor(
          Organization, 
          levels = c(input$org1_id, input$org2_id, input$org3_id, input$org4_id), 
          ordered = TRUE
        ),
        Persona = factor(
          Persona, 
