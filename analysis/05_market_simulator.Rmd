...
##### Updated the beta's to be just the betas from the public_political_social_chartiy_demo Thetas

beta <- public_political_social_charity_demo
...
renderPlotly({
  choice = df()
  ai_exp_util <- round(exp(t(choice[, 3]) %*% as.matrix(intercept_noncentered[,2])),2)
  gp_exp_util <- round(exp(t(choice[, 4]) %*% as.matrix(intercept_noncentered[,2])),2)
  ox_exp_util <- round(exp(t(choice[, 5]) %*% as.matrix(intercept_noncentered[,2])),2)
  rc_exp_util <- round(exp(t(choice[, 6]) %*% as.matrix(intercept_noncentered[,2])),2)
