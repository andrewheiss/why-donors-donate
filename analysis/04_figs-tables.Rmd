...
```{r}
#storing intercept draws for easier reference
draws_intercepts <- draws
```
...
contrasts_int <- tibble(
  ...
  accountability = draws %>% filter(i == 9) %>% select(Gamma) %>% pull(),
  ...
  mutate(
    oxfam_amnesty_criticized = (oxfam + criticized + oxfam_criticized) - (amnesty + criticized + amnesty_criticized),
    
    oxfam_greenpeace_criticized = (oxfam + criticized + oxfam_criticized) - (greenpeace + criticized + greenpeace_criticized),
    
    red_cross_amnesty_criticized = (red_cross + criticized + red_cross_criticized) - (amnesty + criticized + amnesty_criticized),
    
    red_cross_greenpeace_criticized = (red_cross + criticized + red_cross_criticized) - (greenpeace + criticized + greenpeace_criticized),
    
    oxfam_amnesty_crackdown = (oxfam + crackdown + oxfam_crackdown) - (amnesty + crackdown +  amnesty_crackdown),
    
    oxfam_greenpeace_crackdown = (oxfam + crackdown + oxfam_crackdown) - (greenpeace + crackdown +  greenpeace_crackdown),
    
    red_cross_amnesty_crackdown = (red_cross + crackdown + red_cross_crackdown) - (amnesty + crackdown +  amnesty_crackdown),
    
    red_cross_greenpeace_crackdown = (red_cross + crackdown + red_cross_crackdown) - (greenpeace + crackdown + greenpeace_crackdown)
  )

# Plot the contrasts
contrasts_int %>%
  pivot_longer(oxfam_amnesty_criticized:red_cross_greenpeace_crackdown, names_to = "level", values_to = "Gamma") %>% 
  ggplot(aes(x = Gamma, y = level)) +
  stat_halfeye(.width = .95) +
  geom_vline(xintercept = 0, color = "grey") +
  labs(y = "Difference in Attribute Levels", x = "Difference in Gammas")
...
contrasts_int <- tibble(
  ...
  criticized_humanrights = draws %>% filter(i == 15) %>% select(Gamma) %>% pull(),
  criticized_refugee = draws %>% filter(i == 16) %>% select(Gamma) %>% pull(),
  crackdown_environment = draws %>% filter(i == 17) %>% select(Gamma) %>% pull(),
  crackdown_humanrights = draws %>% filter(i == 18) %>% select(Gamma) %>% pull(),
  crackdown_refugee = draws %>% filter(i == 19) %>% select(Gamma) %>% pull(),
) %>% 
  mutate(
    criticized_emergency_environment = (criticized) - (criticized + environment + criticized_environment),
  
  criticized_emergency_humanrights = (criticized) - (criticized + human_rights + criticized_humanrights),
  
  crackdown_emergency_environment = (crackdown) - (crackdown + environment + crackdown_environment),
  
  crackdown_emergency_humanrights = (crackdown) - (crackdown + human_rights + crackdown_humanrights),
  
  criticized_refugee_environment = (criticized + refugee_relief + criticized_refugee) - (criticized + environment + criticized_environment),
  
  criticized_refugee_humanrights = (criticized + refugee_relief + criticized_refugee) - (criticized + human_rights + criticized_humanrights),
  
  crackdown_refugee_environment = (crackdown + refugee_relief + crackdown_refugee) - (crackdown + environment + crackdown_environment),
  
  crackdown_refugee_humanrights = (crackdown + refugee_relief + crackdown_refugee) - (crackdown + human_rights + crackdown_humanrights)
)

# Plot the contrasts
contrasts_int %>%
  pivot_longer(criticized_emergency_environment:crackdown_refugee_humanrights, names_to = "level", values_to = "Gamma") %>% 
  ggplot(aes(x = Gamma, y = level)) +
  stat_halfeye(.width = .95) +
  geom_vline(xintercept = 0, color = "grey") +
  labs(y = "Difference in Attribute Levels", x = "Difference in Gammas")
...
contrasts_int <- tibble(
  ...
  criticized_wealthydonors = (criticized) - (criticized + few_wealthy_private_donors + criticized_wealthydonors),
  
  criticized_grants = (criticized) - (criticized + government_grants + criticized_grants),
  
  crackdown_wealthydonors = (crackdown) - (crackdown + few_wealthy_private_donors + crackdown_wealthydonors),
  
  crackdown_grants = crackdown - (crackdown + government_grants + crackdown_grants)
)

# Plot the contrasts
contrasts_int %>%
  pivot_longer(criticized_wealthydonors:crackdown_grants, names_to = "level", values_to = "Gamma") %>% 
  ggplot(aes(x = Gamma, y = level)) +
  stat_halfeye(.width = .95) +
  geom_vline(xintercept = 0, color = "grey") +
  labs(y = "Difference in Attribute Levels", x = "Difference in Gammas")
...
All of the contrasts are positive and none of them contain 0 within their credibility interval, which supports our hypothesis that donors are more willing to donate to organizations supported by many small private donors and are under criticism or crackdown.
...
# Draws for lower order interactions
...
# Pull the posterior draws for issue area*government crackdown*funding sources interaction
# Reference Level = Funded primarily by many small private donations
# Reference Level = Friendly relationship with government
...
contrasts_int <- tibble(
  ...
  mutate(
  emergency_humanrights_wealthydonors = (crackdown) - (crackdown + few_wealthy_private_donors + human_rights + crackdown_humanrights + crackdown_wealthydonors + humanrights_crackdown_wealthydonors),

  emergency_humanrights_grants = (crackdown) - (crackdown + government_grants + human_rights + crackdown_humanrights + crackdown_grants + humanrights_crackdown_grants),

  emergency_environment_wealthydonors = (crackdown) - (crackdown + few_wealthy_private_donors + environment + crackdown_environment + crackdown_wealthydonors + environment_crackdown_wealthydonors),
  
  emergency_environment_grants = (crackdown) - (crackdown + government_grants + environment + crackdown_environment + crackdown_grants + environment_crackdown_grants),
                                    
  refugee_humanrights_wealthydonors = (crackdown + refugee_relief + crackdown_refugee) - (crackdown + few_wealthy_private_donors + human_rights + crackdown_humanrights + crackdown_wealthydonors + humanrights_crackdown_wealthydonors),

  refugee_humanrights_grants = (crackdown + refugee_relief + crackdown_refugee) - (crackdown + government_grants + human_rights + crackdown_humanrights + crackdown_grants + humanrights_crackdown_grants),

  refugee_environment_wealthydonors = (crackdown + refugee_relief + crackdown_refugee) - (crackdown + few_wealthy_private_donors + environment + crackdown_environment + crackdown_wealthydonors + environment_crackdown_wealthydonors),

  refugee_environment_grants = (crackdown + refugee_relief + crackdown_refugee) - (crackdown + government_grants + environment + crackdown_environment + crackdown_grants + environment_crackdown_grants),
  )

# Plot the contrasts
contrasts_int %>%
  pivot_longer(emergency_humanrights_wealthydonors:refugee_environment_grants, names_to = "level", values_to = "Gamma") %>% 
  ggplot(aes(x = Gamma, y = level)) +
  stat_halfeye(.width = .95) +
  geom_vline(xintercept = 0, color = "grey") +
  labs(y = "Difference in Attribute Levels", x = "Difference in Gammas")
...
Contrast is most likely improperly coded, gammas are all over the place, so no conclusions can be made regarding the hypothesis
