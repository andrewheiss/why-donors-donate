...
contrasts <- tibble(
  financial_transparency = draws %>% filter(level == "Handful of wealthy private donors") %>% select(Gamma) %>% pull(),
  accountibility = draws %>% filter(level == "Government grants") %>% select(Gamma) %>% pull()
  )
