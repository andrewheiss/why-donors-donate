...
draws_4b <- read_rds(here::here("data", "raw_data", "posterior_draws", "interactions_4b.rds"))
draws_3b <- read_rds(here::here("data", "raw_data", "posterior_draws", "interactions_3b.rds"))
...
contrasts_int <- tibble(
  ...
  government_grants = draws %>% filter(i == 11) %>% select(Gamma) %>% pull(),
  crackdown = draws %>% filter(i == 13) %>% select(Gamma) %>% pull(),
  crackdown_environment = draws_3b %>% filter(i == 17) %>% select(Gamma) %>% pull(),
  crackdown_humanrights = draws_3b %>% filter(i == 18) %>% select(Gamma) %>% pull(),
  crackdown_refugee = draws_3b %>% filter(i == 19) %>% select(Gamma) %>% pull(),
  crackdown_wealthydonors = draws_4b %>% filter(i == 16) %>% select(Gamma) %>% pull(),
  crackdown_grants = draws_4b %>% filter(i == 17) %>% select(Gamma) %>% pull(),
  ...
  mutate(
    "crackdown emergency small donors - crackdown human rights wealthy donors" = 
      (crackdown) - 
      (crackdown + few_wealthy_private_donors + human_rights + crackdown_humanrights + crackdown_wealthydonors +
         humanrights_crackdown_wealthydonors),
    "crackdown emergency small donors - crackdown human rights government grants" = 
      (crackdown) - 
      (crackdown + government_grants + human_rights + crackdown_humanrights + crackdown_grants +
         humanrights_crackdown_grants),
    emergency_environment_wealthydonors = 
      (crackdown) - 
      (crackdown + few_wealthy_private_donors + environment + crackdown_environment + crackdown_wealthydonors +
         environment_crackdown_wealthydonors),
    emergency_environment_grants = 
      (crackdown) - 
      (crackdown + government_grants + environment + crackdown_environment + crackdown_grants +
         environment_crackdown_grants),
    refugee_humanrights_wealthydonors = 
      (crackdown + refugee_relief + crackdown_refugee) - 
      (crackdown + few_wealthy_private_donors + human_rights + crackdown_humanrights + crackdown_wealthydonors +
         humanrights_crackdown_wealthydonors),
    refugee_humanrights_grants = 
      (crackdown + refugee_relief + crackdown_refugee) - 
      (crackdown + government_grants + human_rights + crackdown_humanrights + crackdown_grants +
         humanrights_crackdown_grants),
    refugee_environment_wealthydonors = 
      (crackdown + refugee_relief + crackdown_refugee) - 
      (crackdown + few_wealthy_private_donors + environment + crackdown_environment + crackdown_wealthydonors +
         environment_crackdown_wealthydonors),
    refugee_environment_grants = 
      (crackdown + refugee_relief + crackdown_refugee) - 
      (crackdown + government_grants + environment + crackdown_environment + crackdown_grants +
         environment_crackdown_grants)
  )
...
contrasts_int %>%
  pivot_longer(
    emergency_humanrights_wealthydonors:refugee_environment_grants, 
    names_to = "level", values_to = "Gamma"
  ) %>% 
  mutate(level = fct_rev(fct_inorder(level))) %>%
  ggplot(aes(x = Gamma, y = level)) +
...
holding every other attribute constant
...
# Pull the relevant posterior draws
draws <- read_rds(here::here("data", "raw_data", "posterior_draws", "intercept.rds"))

# Compute the contrasts
contrasts <- tibble(
  fin_transparency = draws %>% filter(level == "Financial transparency") %>% select(Gamma) %>% pull(),
...
# Plot the contrasts
contrasts %>%
  pivot_longer(fin_transparency, names_to = "level", values_to = "Gamma") %>% 
  mutate(level = fct_rev(fct_inorder(level))) %>%
  ggplot(aes(x = Gamma, y = level)) +
  stat_halfeye(.width = .95) +
  geom_vline(xintercept = 0, color = "grey") +
...
# Overwrite draws with the appropriate interaction data
draws <- read_rds(here::here("data", "raw_data", "posterior_draws", "interactions_5b.rds"))

# Pull the posterior draws for organizational practices*government crackdown interaction
# Reference Level = Friendly relationship with government
...
contrasts_int <- tibble(
  ...
  fin_transparency_crackdown = draws %>% filter(i == 15) %>% select(Gamma) %>% pull(),
    ...
    fin_transparency_criticized = (fin_transparency + criticized + fin_transparency_criticized) - (criticized),
    fin_transparency_crackdown = (fin_transparency + crackdown + fin_transparency_crackdown) - (crackdown)
  )

# Plot the contrasts
contrasts_int %>%
  pivot_longer(financial_transparency_criticized:financial_transparency_crackdown, names_to = "level", values_to = "Gamma") %>% 
  mutate(level = fct_rev(fct_inorder(level))) %>%
  ggplot(aes(x = Gamma, y = level)) +
  stat_halfeye(.width = .95) +
  geom_vline(xintercept = 0, color = "grey") +
...
The contrasts for hypothesis 5b provide evidence that donors are more willing to donate to organizations which are criticized or under crackdown and are also financially transparent.
...
# Lower order interaction draws
draws_5b <- read_rds(here::here("data", "raw_data", "posterior_draws", "interactions_5b.rds"))
draws_4b <- read_rds(here::here("data", "raw_data", "posterior_draws", "interactions_4b.rds"))

# Pull the posterior draws for organizational practices*funding sources*government crackdown interaction
# Reference Level = Friendly relationship with government
# Reference Level = Funded primarily by many small private donations
# All interactions include financial transparency
...
contrasts_int <- tibble(
  ...
  criticized = draws %>% filter(i == 12)%>% select(Gamma) %>% pull(),
  crackdown = draws %>% filter(i == 13) %>% select(Gamma) %>% pull(),
  criticized_wealthydonors = draws_4b %>% filter(i == 14) %>% select(Gamma) %>% pull(),
  criticized_grants = draws_4b %>% filter(i == 15) %>% select(Gamma) %>% pull(),
  crackdown_wealthydonors = draws_4b %>% filter(i == 16) %>% select(Gamma) %>% pull(),
  crackdown_grants = draws_4b %>% filter(i == 17) %>% select(Gamma) %>% pull(),
  fin_transparency_criticized = draws_5b %>% filter(i == 14) %>% select(Gamma) %>% pull(),
  fin_transparency_crackdown = draws_5b %>% filter(i == 15) %>% select(Gamma) %>% pull(),
  ...
  mutate(
  fin_smalldonors_wealthydonors_criticized = (fin_transparency + criticized + fin_transparency_criticized + fin_smalldonors_criticized) - (fin_transparency + criticized + few_wealthy_private_donors + criticized_wealthydonors + fin_transparency_criticized + fin_wealthydonors_criticized),
  
  fin_smalldonors_grants_criticized = (fin_transparency + criticized + fin_transparency_criticized + fin_smalldonors_criticized) -  (fin_transparency + criticized + government_grants + criticized_grants + fin_transparency_criticized + fin_grants_criticized),
  
  fin_smalldonors_wealthydonors_crackdown = (fin_transparency + crackdown + fin_transparency_crackdown + fin_smalldonors_crackdown) - (fin_transparency + crackdown + few_wealthy_private_donors + crackdown_wealthydonors + fin_transparency_crackdown + fin_wealthydonors_crackdown),
  
  fin_smalldonors_grants_crackdown = (fin_transparency + crackdown + fin_transparency_crackdown + fin_smalldonors_crackdown) - (fin_transparency + crackdown + government_grants + crackdown_grants + fin_transparency_crackdown + fin_grants_crackdown),

# additional contrasts to compare financial transparency to no transparency    
  not_fin_smalldonors_wealthydonors_criticized = (fin_transparency + criticized + fin_transparency_criticized + fin_smalldonors_criticized) - (criticized + few_wealthy_private_donors + criticized_wealthydonors),
  
  not_fin_smalldonors_grants_criticized = (fin_transparency + criticized + fin_transparency_criticized + fin_smalldonors_criticized) -  (criticized + government_grants + criticized_grants),
  
  not_fin_smalldonors_wealthydonors_crackdown = (fin_transparency + crackdown + fin_transparency_crackdown + fin_smalldonors_crackdown) - (crackdown + few_wealthy_private_donors + crackdown_wealthydonors),
  
  not_fin_smalldonors_grants_crackdown = (fin_transparency + crackdown + fin_transparency_crackdown + fin_smalldonors_crackdown) - (crackdown + government_grants + fin_transparency_crackdown + fin_grants_crackdown)
  )

# Plot the contrasts
contrasts_int %>%
  pivot_longer(fin_smalldonors_wealthydonors_criticized:not_fin_smalldonors_grants_crackdown, names_to = "level", values_to = "Gamma") %>% 
  mutate(level = fct_rev(fct_inorder(level))) %>%
  ggplot(aes(x = Gamma, y = level)) +
  stat_halfeye(.width = .95) +
  geom_vline(xintercept = 0, color = "grey") +
...
Only fin_smalldonors_grants_crackdown contains 0 within its credibility interval. Since all other contrasts are positive and do not contain 0 within their credibility intervals, there is evidence supporting that donors are more willing to donate to NGOs that are criticized by the government/under government crackdown, are financially transparent, and are funded primarily by numerous small private donors.
...
# Pull the posterior draws for organizational practices*issue area*government crackdown interaction
# Reference Level = Friendly relationship with government
# Reference Level = Emergency Response
# All interactions include financial transparency
...
contrasts_int <- tibble(
  ...
  human_rights = draws %>% filter(i == 6) %>% select(Gamma) %>% pull(),
  refugee_relief = draws %>% filter(i == 7) %>% select(Gamma) %>% pull(),
  criticized_environment = draws_3b %>% filter(i == 14) %>% select(Gamma) %>% pull(),
  criticized_humanrights = draws_3b %>% filter(i == 15) %>% select(Gamma) %>% pull(),
  criticized_refugee = draws_3b %>% filter(i == 16) %>% select(Gamma) %>% pull(),
  crackdown_environment = draws_3b %>% filter(i == 17) %>% select(Gamma) %>% pull(),
  crackdown_humanrights = draws_3b %>% filter(i == 18) %>% select(Gamma) %>% pull(),
  crackdown_refugee = draws_3b %>% filter(i == 19) %>% select(Gamma) %>% pull(),
  fin_transparency_criticized = draws_5b %>% filter(i == 14) %>% select(Gamma) %>% pull(),
  ...
  mutate(
  fin_emergency_environment_criticized = (fin_transparency + criticized + fin_transparency_criticized + fin_emergency_criticized) - (fin_transparency + criticized + environment + fin_transparency_criticized + criticized_environment + fin_environment_criticized),
  
  fin_emergency_humanrights_criticized = (fin_transparency + criticized + fin_transparency_criticized + fin_emergency_criticized) - (fin_transparency + criticized + human_rights + fin_transparency_criticized + criticized_humanrights + fin_humanrights_criticized),
  
  fin_emergency_environment_crackdown = (fin_transparency + crackdown + fin_transparency_crackdown + fin_emergency_crackdown) - (fin_transparency + crackdown + environment + fin_transparency_crackdown + crackdown_environment + fin_environment_crackdown),
  
  fin_emergency_humanright_crackdown = (fin_transparency + crackdown + fin_transparency_crackdown + fin_emergency_crackdown) - (fin_transparency + crackdown + human_rights + fin_transparency_crackdown + crackdown_humanrights + fin_humanrights_crackdown),
  
  fin_refugee_environment_criticized = (fin_transparency + criticized + fin_transparency_criticized + criticized_refugee + fin_refugee_criticized) - (fin_transparency + criticized + environment + fin_transparency_criticized + criticized_environment + fin_environment_criticized),
  
  fin_refugee_humanright_criticized = (fin_transparency + criticized + fin_transparency_criticized + criticized_refugee + fin_refugee_criticized) - (fin_transparency + criticized + human_rights + fin_transparency_criticized + criticized_humanrights + fin_humanrights_criticized),
  
  fin_refugee_environment_crackdown = (fin_transparency + crackdown + fin_transparency_crackdown + crackdown_refugee + fin_refugee_crackdown) - (fin_transparency + crackdown + environment + crackdown_environment + fin_transparency_crackdown + fin_environment_crackdown),
  
  fin_refugee_humanright_crackdown = (fin_transparency + crackdown + fin_transparency_crackdown + crackdown_refugee + fin_refugee_crackdown) - (fin_transparency + crackdown + human_rights + fin_transparency_crackdown + crackdown_humanrights + fin_humanrights_crackdown)
)

# Plot the contrasts
contrasts_int %>%
  pivot_longer(fin_emergency_environment_criticized:fin_refugee_humanright_crackdown, names_to = "level", values_to = "Gamma") %>% 
  mutate(level = fct_rev(fct_inorder(level))) %>%
  ggplot(aes(x = Gamma, y = level)) +
  stat_halfeye(.width = .95) +
  geom_vline(xintercept = 0, color = "grey") +
...
The contrasts don't seem to be properly calculated, so no conclusion at this time.
...
draws_5b <- read_rds(here::here("data", "raw_data", "posterior_draws", "interactions_5b.rds"))
draws_3b <- read_rds(here::here("data", "raw_data", "posterior_draws", "interactions_3b.rds"))
draws_5c <- read_rds(here::here("data", "raw_data", "posterior_draws", "interactions_5c.rds"))
draws_5d <- read_rds(here::here("data", "raw_data", "posterior_draws", "interactions_5d.rds"))
# Pull the posterior draws for organizational practices*funding*issue area*government crackdown interaction
# Reference Level = Friendly relationship with government
# Reference Level = Emergency Response
# All interactions include small donors
# All interactions include financial transparency
...
contrasts_int <- tibble(
  ...
  fin_transparency_criticized = draws_5b %>% filter(i == 14) %>% select(Gamma) %>% pull(),
  fin_transparency_crackdown = draws_5b %>% filter(i == 15) %>% select(Gamma) %>% pull(),
  fin_emergency_criticized = draws_5d %>% filter(i == 14) %>% select(Gamma) %>% pull(),
  fin_emergency_crackdown = draws_5d %>% filter(i == 15) %>% select(Gamma) %>% pull(),
  fin_environment_criticized = draws_5d %>% filter(i == 16) %>% select(Gamma) %>% pull(),
  fin_environment_crackdown = draws_5d %>% filter(i == 17) %>% select(Gamma) %>% pull(),
  fin_humanrights_criticized = draws_5d %>% filter(i == 18) %>% select(Gamma) %>% pull(),
  fin_humanrights_crackdown = draws_5d %>% filter(i == 19) %>% select(Gamma) %>% pull(),
  fin_refugee_criticized = draws_5d %>% filter(i == 20) %>% select(Gamma) %>% pull(),
  fin_refugee_crackdown = draws_5d %>% filter(i == 21) %>% select(Gamma) %>% pull(),
  fin_smalldonors_criticized = draws_5c %>% filter(i == 14) %>% select(Gamma) %>% pull(),
  fin_wealthydonors_criticized = draws_5c %>% filter(i == 15) %>% select(Gamma) %>% pull(),
  fin_grants_criticized = draws_5c %>% filter(i == 16) %>% select(Gamma) %>% pull(),
  fin_smalldonors_crackdown = draws_5c %>% filter(i == 17) %>% select(Gamma) %>% pull(),
  fin_wealthydonors_crackdown = draws_5c %>% filter(i == 18) %>% select(Gamma) %>% pull(),
  fin_grants_crackdown = draws_5c %>% filter(i == 19) %>% select(Gamma) %>% pull(),
  fin_sd_emergency_criticized = draws %>% filter(i == 14) %>% select(Gamma) %>% pull(),
  fin_sd_emergency_crackdown = draws %>% filter(i == 15) %>% select(Gamma) %>% pull(),
  fin_sd_environment_criticized = draws %>% filter(i == 16) %>% select(Gamma) %>% pull(),
  fin_sd_environment_crackdown = draws %>% filter(i == 17) %>% select(Gamma) %>% pull(),
  fin_sd_humanrights_criticized = draws %>% filter(i == 18) %>% select(Gamma) %>% pull(),
  fin_sd_humanrights_crackdown = draws %>% filter(i == 19) %>% select(Gamma) %>% pull(),
  fin_sd_refugee_criticized = draws %>% filter(i == 20) %>% select(Gamma) %>% pull(),
  fin_sd_refugee_crackdown = draws %>% filter(i == 21) %>% select(Gamma) %>% pull()
  ) %>% mutate(
  fin_sd_emergency_environment_criticized = (fin_transparency + criticized + fin_transparency_criticized + fin_smalldonors_criticized + fin_emergency_criticized + fin_sd_emergency_criticized) - (fin_transparency + criticized + environment + criticized_environment + fin_transparency_criticized + fin_smalldonors_criticized + fin_environment_criticized + fin_sd_environment_criticized),
  
  fin_sd_emergency_humanrights_criticized = (fin_transparency + criticized + fin_transparency_criticized + fin_smalldonors_criticized + fin_emergency_criticized + fin_sd_emergency_criticized) - (fin_transparency + criticized + human_rights + criticized_humanrights + fin_transparency_criticized + fin_smalldonors_criticized + fin_humanrights_criticized + fin_sd_humanrights_criticized) ,
  
  fin_sd_emergency_environment_crackdown = (fin_transparency + crackdown + fin_transparency_crackdown + fin_smalldonors_crackdown + fin_emergency_crackdown + fin_sd_emergency_crackdown) - (fin_transparency + crackdown + environment + crackdown_environment + fin_transparency_crackdown + fin_smalldonors_crackdown + fin_environment_crackdown + fin_sd_environment_crackdown),
  
  fin_sd_emergency_humanrights_crackdown = (fin_transparency + crackdown + fin_transparency_crackdown + fin_smalldonors_crackdown + fin_emergency_crackdown + fin_sd_emergency_crackdown) - (fin_transparency + crackdown + human_rights + crackdown_humanrights + fin_transparency_crackdown + fin_smalldonors_crackdown + fin_humanrights_crackdown + fin_sd_humanrights_crackdown) ,
  
  fin_sd_refugee_environment_criticized = (fin_transparency + criticized + refugee_relief + fin_transparency_criticized + criticized_refugee + fin_smalldonors_criticized + fin_refugee_criticized + fin_sd_refugee_criticized) - (fin_transparency + criticized + environment + criticized_environment + fin_transparency_criticized + fin_smalldonors_criticized + fin_environment_criticized + fin_sd_environment_criticized) ,
  
  fin_sd_refugee_humanrights_criticized = (fin_transparency + criticized + refugee_relief + fin_transparency_criticized + criticized_refugee + fin_smalldonors_criticized + fin_refugee_criticized + fin_sd_refugee_criticized) - (fin_transparency + criticized + human_rights + criticized_humanrights + fin_transparency_criticized + fin_smalldonors_criticized + fin_humanrights_criticized + fin_sd_humanrights_criticized) ,
  
  fin_sd_refugee_environment_crackdown = (fin_transparency + crackdown + refugee_relief + fin_transparency_crackdown + crackdown_refugee + fin_smalldonors_crackdown + fin_refugee_crackdown + fin_sd_refugee_crackdown) - (fin_transparency + crackdown + environment + crackdown_environment + fin_transparency_crackdown + fin_smalldonors_crackdown + fin_environment_crackdown + fin_sd_environment_crackdown) ,
  
  fin_sd_refugee_humanrights_crackdown = (fin_transparency + crackdown + refugee_relief + fin_transparency_crackdown + crackdown_refugee + fin_smalldonors_crackdown + fin_refugee_crackdown + fin_sd_refugee_crackdown) - (fin_transparency + crackdown + human_rights + crackdown_humanrights + fin_transparency_crackdown + fin_smalldonors_crackdown + fin_humanrights_crackdown + fin_sd_humanrights_crackdown),
  )

# Plot the contrasts
contrasts_int %>%
  pivot_longer(fin_sd_emergency_environment_criticized:fin_sd_refugee_humanrights_crackdown, names_to = "level", values_to = "Gamma") %>% 
  mutate(level = fct_rev(fct_inorder(level))) %>%
  ggplot(aes(x = Gamma, y = level)) +
  stat_halfeye(.width = .95) +
  geom_vline(xintercept = 0, color = "grey") +
...
These contrasts don't provide conclusive results, need fixing.

holding every other attribute constant

#### Hypothesis 5f, Organizational Practices
