...
# Plot the contrasts
contrasts_int %>%
  pivot_longer(criticized_refugee_environment:crackdown_refugee_humanrights, names_to = "level", values_to = "Gamma") %>% 
  ggplot(aes(x = Gamma, y = level)) +
  stat_halfeye(.width = .95) +
  geom_vline(xintercept = 0, color = "grey") +
  labs(y = "Difference in Attribute Levels", x = "Difference in Gammas")
...
contrasts_int <- tibble(
  ...
  criticized_wealthydonors = (criticized + few_wealthy_private_donors + criticized_wealthydonors) - criticized,
  
  criticized_grants = (criticized + government_grants + criticized_grants) - criticized,
  
  crackdown_wealthydonors = (crackdown + few_wealthy_private_donors + crackdown_wealthydonors) - crackdown,
  
  crackdown_grants = (crackdown + government_grants + crackdown_grants) - crackdown
)
...
contrasts_int <- tibble(
  ...
  mutate(
  emergency_humanrights_wealthydonors = (crackdown + emergency_crackdown_wealthydonors) - (crackdown + few_wealthy_private_donors + human_rights + humanrights_crackdown_wealthydonors),
  
  emergency_humanrights_grants = (crackdown + emergency_crackdown_grants) - (crackdown + government_grants + human_rights + humanrights_crackdown_grants),
  
  emergency_environment_wealthydonors = (crackdown + emergency_crackdown_wealthydonors) - (crackdown + few_wealthy_private_donors + environment + environment_crackdown_wealthydonors),
  
  emergency_environment_grants = (crackdown + emergency_crackdown_grants) - (crackdown + government_grants + environment + environment_crackdown_wealthydonors),
                                    
  refugee_humanrights_wealthydonors = (crackdown + refugee_relief + refugee_crackdown_wealthydonors) - (crackdown + few_wealthy_private_donors + human_rights + humanrights_crackdown_wealthydonors),
  
  refugee_humanrights_grants = (crackdown + refugee_relief + refugee_crackdown_grants) - (crackdown + government_grants +  human_rights + humanrights_crackdown_grants),
    
  refugee_environment_wealthydonors = (crackdown + refugee_relief + refugee_crackdown_wealthydonors) - (crackdown + few_wealthy_private_donors + environment + environment_crackdown_wealthydonors),

  refugee_environment_grants = (crackdown + refugee_relief + refugee_crackdown_grants) - (crackdown + government_grants + environment + environment_crackdown_grants),
...
contrasts_int <- tibble(
  ...
  criticized = draws %>% filter(i == 12)%>% select(Gamma) %>% pull(),
  crackdown = draws %>% filter(i == 13) %>% select(Gamma) %>% pull(),
  financial_transparency_criticized = draws %>% filter(i == 14) %>% select(Gamma) %>% pull(),
  financial_transparency_crackdown = draws %>% filter(i == 15) %>% select(Gamma) %>% pull(),
  ) %>% mutate(
    financial_transparency_criticized = fin_transparency + criticized + financial_transparency_criticized,
    financial_transparency_crackdown = fin_transparency + crackdown + financial_transparency_crackdown
  )
...
contrasts_int <- tibble(
  ...
  mutate(
  fin_smalldonors_wealthydonors_criticized = (fin_transparency + criticized + fin_smalldonors_criticized) - (fin_transparency + criticized + few_wealthy_private_donors + fin_wealthydonors_criticized),
  
  fin_smalldonors_grants_criticized = (fin_transparency + criticized + fin_smalldonors_criticized) -  (fin_transparency + criticized + government_grants + fin_grants_criticized),
  
  fin_smalldonors_wealthydonors_crackdown = (fin_transparency + crackdown + fin_smalldonors_crackdown) - (fin_transparency + crackdown + few_wealthy_private_donors + fin_wealthydonors_crackdown),
  
  fin_smalldonors_grants_crackdown = (fin_transparency + crackdown + fin_smalldonors_crackdown) - (fin_transparency + crackdown + government_grants + fin_grants_crackdown)
  )

# Plot the contrasts
contrasts_int %>%
  pivot_longer(fin_smalldonors_wealthydonors_criticized:fin_smalldonors_grants_crackdown, names_to = "level", values_to = "Gamma") %>% 
  ggplot(aes(x = Gamma, y = level)) +
  stat_halfeye(.width = .95) +
  geom_vline(xintercept = 0, color = "grey") +
  labs(y = "Difference in Attribute Levels", x = "Difference in Gammas")
...
contrasts_int <- tibble(
  ...
  mutate(
  fin_emergency_environment_criticized = (fin_transparency + criticized + fin_emergency_criticized) - (fin_transparency + criticized + environment + fin_environment_criticized),
  
  fin_emergency_humanrights_criticized = (fin_transparency + criticized + fin_emergency_criticized) - fin_transparency + criticized + human_rights + fin_humanrights_criticized,
  
  fin_emergency_environment_crackdown = (fin_transparency + crackdown + fin_emergency_crackdown) - (fin_transparency + crackdown + environment + fin_environment_crackdown),
  
  fin_emergency_humanright_crackdown = (fin_transparency + crackdown + fin_emergency_crackdown) - (fin_transparency + crackdown + human_rights + fin_humanrights_crackdown),
  
  fin_refugee_environment_criticized = (fin_transparency + criticized + fin_refugee_criticized) - (fin_transparency + criticized + environment + fin_environment_criticized),
  
  fin_refugee_humanright_criticized = (fin_transparency + criticized + fin_refugee_criticized) - (fin_transparency + criticized + human_rights + fin_humanrights_criticized),
  
  fin_refugee_environment_crackdown = (fin_transparency + crackdown + fin_refugee_crackdown) - (fin_transparency + crackdown + environment + fin_environment_crackdown),
  
  fin_refugee_humanright_crackdown = (fin_transparency + crackdown + fin_refugee_crackdown) - (fin_transparency + crackdown + human_rights + fin_humanrights_crackdown)
)
...
contrasts_int <- tibble(
  ...
  fin_sd_emergency_environment_criticized = (fin_transparency + criticized + fin_sd_emergency_criticized) - (fin_transparency + criticized + environment +  fin_sd_environment_criticized),
  
  fin_sd_emergency_humanrights_criticized = (fin_transparency + criticized + fin_sd_emergency_criticized) - (fin_transparency + criticized + human_rights +  fin_sd_humanrights_criticized),
  
  fin_sd_emergency_environment_crackdown = (fin_transparency + crackdown + fin_sd_emergency_crackdown) - (fin_transparency + crackdown + environment +  fin_sd_environment_crackdown),
  
  fin_sd_emergency_humanright_crackdown = (fin_transparency + crackdown + fin_sd_emergency_crackdown) - (fin_transparency + crackdown + human_rights + fin_sd_humanrights_crackdown),
  
  fin_sd_refugee_environment_criticized = (fin_transparency + criticized + refugee_relief +  fin_sd_refugee_criticized) - (fin_transparency + criticized + environment +  fin_sd_environment_criticized),
  
  fin_sd_refugee_humanright_criticized = (fin_transparency + criticized + refugee_relief +  fin_sd_refugee_criticized) - (fin_transparency + criticized + human_rights +  fin_sd_humanrights_criticized),
  
  fin_sd_refugee_environment_crackdown = (fin_transparency + crackdown + refugee_relief + fin_sd_refugee_crackdown) - (fin_transparency + crackdown + environment + fin_sd_environment_crackdown),
  
  fin_sd_refugee_humanright_crackdown = (fin_transparency + crackdown + refugee_relief + fin_sd_refugee_crackdown) - (fin_transparency + crackdown + human_rights + fin_sd_humanrights_crackdown
))

# Plot the contrasts
contrasts_int %>%
  pivot_longer(fin_sd_emergency_environment_criticized:fin_sd_refugee_humanright_crackdown, names_to = "level", values_to = "Gamma") %>% 
  ggplot(aes(x = Gamma, y = level)) +
  stat_halfeye(.width = .95) +
  geom_vline(xintercept = 0, color = "grey") +
  labs(y = "Difference in Attribute Levels", x = "Difference in Gammas")
