...
The following analysis follows specifying the model we'd like to evaluate. Note that the output of the model calibration is too large to push to GitHub.

## Load model output

```{r load-data-output, warning=FALSE, message=FALSE, eval=FALSE}
# Load all model output
intercept_run <- read_rds(here::here("analysis", "output", "model_runs", "intercept_noncentered.rds"))
...
# Load draws and plot marginals and contrasts
...
# # Create matrix of Gammas.
# gamma_draws <- draws %>% 
#   group_by(i, j) %>% 
#   summarize(gamma_mean = mean(Gamma))
# 
# gamma_matrix <- matrix(as.matrix(gamma_draws)[,3], nrow = max(draws$j), ncol = max(draws$i))
# write.csv(gamma_matrix, file = here::here("analysis", "output", "posterior_means.csv"))
