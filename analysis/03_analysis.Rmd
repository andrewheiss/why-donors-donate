...
# ##############################
X_new <- array(NA, dim = c(dim(X)[1:3], 19)) # 13 + interactions
for (r in 1:dim(X)[1]) {
  for (s in 1:dim(X)[2]) {
    X_new[r,s,,] <- X[r,s,,c(1:13, 22:27)] # 13 + interactions
  }
}
X <- X_new
# ##############################
...
data <- list(
  ...
  Omega_shape = 1,     # Shape of population-level scale
...
if (intercept == 1 & interactions == 1) write_rds(run, here::here("analysis", "output", "model_runs", "interactions_3b.rds"))
...
if (intercept == 1 & interactions == 1) run <- read_rds(here::here("analysis", "output", "model_runs", "interactions.rds"))
...
if (intercept == 1 & interactions == 1) write_rds(draws, here::here("analysis", "output", "posterior_draws", "interactions.rds"))
